...
// const aircode = require("aircode");
 // "aircode": "^0.5.0",这个包暂时不导入
 const lark = require("@larksuiteoapi/node-sdk");
 var axios = require("axios");
 // const EventDB = aircode.db.table("event");//暂时注释
 // const MsgTable = aircode.db.table("msg"); // 用于保存历史会话的表
 
 // 如果你不想配置环境变量，或环境变量不生效，则可以把结果填写在每一行最后的 "" 内部
 const FEISHU_APP_ID = process.env.APPID || "cli_a6882c8dc8ff900e"; // 飞书的应用 ID
 const FEISHU_APP_SECRET = process.env.SECRET || "l0kymcHk7BHRBDsFDdBBNf5dXZHWLhvS"; // 飞书的应用的 Secret
 ...
 async function buildConversation(sessionId, question) {
   ...
   // 从 MsgTable 表中取出历史记录构造 question 暂时注释
 ...
 async function saveConversation(sessionId, question, answer) {
   ...
   // const result = await MsgTable.save({暂时注释
   //   sessionId,
   //   question,
   //   answer,
   //   msgSize,
 ...
 // async function discardConversation(sessionId) { 暂时注释
 //   let totalSize = 0;
 //   const countList = [];
 ...
 // 清除历史会话
 // async function clearConversation(sessionId) { 暂时注释
 ...
 async function cmdClear(sessionId, messageId) {
   // await clearConversation(sessionId)暂时注释
...
//    evt_record.content = userInput.text;
 ...
 module.exports = async function (params, context) {
   ...
   if ((params.header.event_type === "im.message.receive_v1")) {
     ...
     if(eventList.length > MAX_LENGTH){
        var eId = eventList.shift();
        delete eventJson[eId]
     }
     // 对于同一个事件，只处理一次 暂时注释
